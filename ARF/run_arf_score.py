...
def train(args, train_dataset, dev_dataset, qve_model, qa_model, tokenizer):
    ...
    for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            estimated_label = (qa_values >= threshold2).float()  # True→1.0, False→0.0
            ...
            prob_values = torch.sigmoid(qa_values)  # 转换为概率
            select_prob = Binomial(1, prob_values).sample()
            ...
            #threshold = 0.5 + 0.3 * (epoch / max_epoch)  # 从0.5逐步增加到0.8
            # 策略梯度损失
            prob_values = torch.sigmoid(qa_values)  # 将评分转换为概率
            prob = select_prob * torch.log(prob_values + epsilon) + (1 - select_prob) * torch.log(1 - prob_values + epsilon)
...
def estimation(args, tokenizer, qve_model, qa_model):
    ...
    # 保存过滤后的数据集
    json.dump(filtered_data_json, open(os.path.join(args.output_dir, "filtered_score_qa.json"), 'w'))
...
def main():
    ...
    if args.do_estimation:
        ...
        #加载分词器
        tokenizer = AutoTokenizer.from_pretrained(
            args.tokenizer_name if args.tokenizer_name else args.qve_model_name_or_path,#分词器名
